...
- **Step 7 superseded (2026-09-18): quick-add FAB retired, merged into the "Ask Remi" dashboard chat.** The standalone `QuickAddComponent` FAB (step 7c) and its `quick-add-parse` orchestrator template (step 7b) are removed. `family-chat` is now a single hybrid template: every chat turn returns `{ reply, items }` — `items` empty for ordinary questions, populated with the same five-type parsed shape when the user asks to create/save something. Item schema text lives in the shared `ITEM_SCHEMA_BLOCK` constant in `functions/index.js`. `orchestratedGenerate` gained an `'items-field'` `logSuggestion` mode (logs `result.items[]` per-item, replacing the old top-level-array `'per-item'` mode) so each proposed item is still independently accept/edit/reject-able via the existing `AiSuggestionService` API. Client-side, the parsing/creation business logic (type maps, duplicate detection, `createRecord` dispatch) moved from `QuickAddComponent` into a shared `QuickAddCreationService` (`src/app/services/`), and the per-item confirmation-card UI moved into a standalone `QuickAddCardComponent` (`src/app/shared/quick-add-card/`) rendered inline under the relevant chat message in `DashboardComponent`. Duplicate detection and edit-before-confirm behavior are unchanged from step 7d — only the entry point moved.